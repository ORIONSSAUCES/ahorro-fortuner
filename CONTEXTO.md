...
- **Préstamos:** se carga la cuota (fija o variable), el día de vencimiento, las **cuotas que faltan** y las ya pagadas. La app arma un **calendario mensual de cuotas** (`l.cuotas = [{due, amount, paid}]`, generado con `buildSchedule`). Al **tocar el préstamo se despliegan las cuotas**, cada una con su fecha, estado ("Faltan X días" / "Vencida" / "Pagada"), monto **editable por cuota** (para las variables) y botón de pago individual. Barra de avance "Cuota X/Y · faltan Z" (Y = paidPrev + cuotas). El "te sobra" cuenta la cuota que vence en el mes actual (pagada o no). Préstamos viejos (formato total/paid) se migran solos con `migrateLoan`.
